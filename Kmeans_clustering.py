...
wcss=[]
for i in range(1,11):
    cluster=KMeans(n_clusters=i,init='k-means++',max_iter=300,n_init=10)
    cluster.fit(X)
    wcss.append(cluster.inertia_)
plt.plot(range(1,11),wcss)

cluster=KMeans(n_clusters=5,init='k-means++',max_iter=300,n_init=10)
ykmeans=cluster.fit_predict(X)
